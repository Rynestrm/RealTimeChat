...
// every time a user connects it is displayed to all other users
socket.on('userConnected', function(name){
   // puts the users name into the connected message
   displayMessage(`${name} Connected`);
})

//out put the messages to the page.
function displayMessage(message){
   // making a new div for each element
   const MessageDiv = document.createElement('div');
   //this takes the div and puts the message into it
   MessageDiv.innerHTML = message;
   //appending the message to the container
